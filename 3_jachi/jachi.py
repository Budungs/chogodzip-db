...
while(1):
    # 카테고리 데이터 가져오기
    url = f'https://api.gobang.kr/v1/houses?%7B%22HOUSE_TYPE_CDS%22%3A%5B%22HOUTP00008%22%2C%22HOUTP00009%22%5D%2C%22DONGLI_CDS%22%3A%5B%2211%22%5D%2C%22PAGE_NUM%22%3A{cur_page_num}%2C%22randomSeed%22%3A1526%7D'
    ...
    for house in json_data["DATA"]:
       ...
       # -------------------------- ROOM 테이블 데이터 -----------------------
        ...
        house_type_nms = house["HOUSE_TYPE_NMS"] # 검증
        ...
        # SQL 실행 부분 (원,투룸 오피스텔)
        cursor.execute("SELECT COUNT(*) FROM JACHI WHERE ROOM_ID = %s", (room_id,))
        result = cursor.fetchone()

        if result[0] == 0:  # 데이터가 중복되지 않는 경우에만 삽입
            cursor.execute("""
                INSERT INTO JACHI
                (ROOM_ID, PRICE_MIN, PRICE_MAX, DEPOSIT_MIN, DEPOSIT_MAX, MAINTENANCE_FEE, FLOOR, NAME, ROOM_TYPE, DURATION_MIN, TYPE) 
...
cursor.execute("SELECT * FROM JACHI")
