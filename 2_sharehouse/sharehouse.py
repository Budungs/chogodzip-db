...
while(1):
    ...
    for house in json_data["DATA"]:
        ...
        room_cnt = house["ROOM_CNT"]
        print(room_cnt)

        accomo_cnt = house["ACCOMO_CNT"]
        print(accomo_cnt)
        ...
        # is_sold_out = random.choices(['T', 'F'], weights=[5, 95])[0] # 'T'(판매완료) 5%, 'F'(판매중) 95%
        # print(is_sold_out)
        ...
        # SQL 실행 부분 (쉐어하우스)
        cursor.execute("SELECT COUNT(*) FROM SHAREHOUSE WHERE ROOM_ID = %s", (room_id,))
        result = cursor.fetchone()

        if result[0] == 0:  # 데이터가 중복되지 않는 경우에만 삽입
            cursor.execute("""
                INSERT INTO SHAREHOUSE
                (ROOM_ID, PRICE_MIN, PRICE_MAX, DEPOSIT_MIN, DEPOSIT_MAX, MAINTENANCE_FEE, GENDER_LIMIT, TYPE, CONTRACT_MIN, AGE_MAX, AGE_MIN, ROOM_CNT, ACCOMO_CNT) 
...
cursor.execute("SELECT * FROM SHAREHOUSE")
